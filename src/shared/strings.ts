/**
 * The UI copy, kept out of the "use client" module that serves it.
 *
 * A server component that imports from a client module gets a reference, not
 * the value — so the playlist pages, which print Playlist names as plain HTML,
 * read the strings from here. Client code still imports them through
 * `@/client/i18n`, which re-exports this.
 */
